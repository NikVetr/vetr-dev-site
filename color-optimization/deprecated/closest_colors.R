...
cart2polar <- function(x,y){c(atan2(y/x), sqrt(x^2 + y^2))}
...
flex_bounds <- function(x, xlim = c(0,1), rat = 1.1){
  ...
  mean_x  <- mean(x)
  new_x   <- mean_x + c(-0.5, 0.5) * range_x * rat
  pmax(pmin(new_x, xlim[2]), xlim[1])
}
...
rescale <- function(M, cs_ranges, color_space) {
  cn <- colnames(M)
  for (cni in cn) {
    M[, cni] <- M[, cni] *
      (cs_ranges[[color_space]]$max[cni] -
...
mean_dist_func <- function(par,
  ...
  new_hex  <- encode_colour(rescale(m, cs_ranges, color_space), from = color_space)
  curr_hex <- encode_colour(rescale(curr_cols, cs_ranges, color_space), from = color_space)
  ...
  if(!is.null(init_weights) & !is.null(init_colors)){
    if(length(init_weights) != nncols){
      init_weights <- rep(init_weights, length.out = nncols)
    }
...
# Optional textual output for integration checks with the JS tool
if (nzchar(Sys.getenv("COLOR_OPT_ECHO"))) {
