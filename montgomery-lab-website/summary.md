...
- `index.html` is a full-viewport concept switcher. It defaults to `montgomery-lab-website_classy/index.html` and swaps the other variants into an iframe through three visually distinct selector buttons: Classic Vanilla, Genome OS, and Stage Rig.
...
- The three concepts now diverge on mobile as well as desktop: Classic Vanilla uses a polished pill header, Genome OS uses a compact neon command header with a toggle drawer, and Stage Rig uses a short ticket-header with a hidden setlist menu.
...
- Recent refinement work also added consortium-specific subpages to the Genome OS and Stage Rig concepts, widened the Genome OS PI lead panel, reduced repeated photo defaults across subpages, and taught the Classic Vanilla consortium detail card to switch between top-mounted wide logos and left-rail tall logos.
