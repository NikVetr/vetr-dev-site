...
- **Compact mobile command header:** on mobile, the Genome OS dock becomes a short top bar with the lab mark on the left and a neon hamburger control on the right; the pipeline links remain hidden until the user opens the drawer.
- **Compact mobile command header:** on mobile, the Genome OS dock becomes a short top bar with the lab mark on the left and a neon hamburger control on the right; the pipeline links remain hidden until the user opens the drawer. The mobile menu bars are now packed more tightly, and the opened drawer uses an opaque dark backing so text is readable over the animated background.
- **Page-varied carousel queues:** carousel image order now includes page and position offsets plus page-specific photo pools, reducing repeated photo sequences across subpages that share the same starting image.
- **Repeated stills reduced in content defaults:** the most obvious repeated default images on the biotech news/publications surfaces were swapped so the concept no longer leans on the same few generated lab scenes quite as heavily.
- **Consortium dossiers and wider lead panels:** the Genome OS build now includes dedicated consortium detail pages under `consortia/*.html`, keeps the mobile drawer fully opaque, and moves the team overview below a wider Stephen Montgomery lead panel so the headshot and copy have more horizontal room.
