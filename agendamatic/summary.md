...
- Tracker renders a slim dotted progress guide from the progress bar to the axis behind chart blocks, keeps the red time marker above the axis labels, and uses solid item-colored connectors for displaced major time labels.
...
- The active tracker block uses per-theme color inversion plus an upward, color-matched glow that fades above the axis; items completed through Next Item, their overflow labels, and their connectors fade until Previous rewinds them.
- The pop-out recomputes axis and overflow label geometry in its own viewport instead of cloning main-panel pixel coordinates, refits status tickers, and preserves complete panel contents by scrolling the window rather than clipping undersized layouts.
