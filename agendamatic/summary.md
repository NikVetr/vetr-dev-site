...
- Current Status tracks the active item continuously (including overtime): before the boundary it shows time left; after the boundary it switches to "PAST THE END OF [item]" while keeping the next-item line. Its number ruler uses whole-minute ticks; longer values are decomposed into a compact hours line above the larger remaining-minute ruler and its `MINUTES` label. Labels translate continuously and interpolate in size and opacity around the exact remaining value, including in the pop-out.
...
- The active tracker block exclusively uses per-theme color inversion plus a faint, block-width rectangular color wash that rises vertically and fades above the axis. Tracker completion styling follows the manual progression boundary: blocks before the active index, along with their overflow labels and connectors, fade until Previous rewinds them, while the active block is never faded even after agenda reordering.
