...
- Tracker renders a two-pixel dotted progress guide above the active-item glow but behind chart blocks, keeps the red time marker above the axis labels, and uses solid item-colored connectors for displaced major time labels.
...
- The active tracker block uses per-theme color inversion plus a minimum-width upward, color-matched glow that remains visible for narrow items and fades above the axis; items completed through Next Item, their overflow labels, and their connectors fade until Previous rewinds them.
- The pop-out recomputes axis and overflow label geometry in its own viewport instead of cloning main-panel pixel coordinates, refits status tickers, and preserves complete panel contents by scrolling the window rather than clipping undersized layouts. Its synchronization preserves existing DOM nodes, updates live marker/progress styles incrementally, and only rebuilds Tracker structure when agenda state or viewport geometry changes.
