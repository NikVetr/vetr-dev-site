...
- Current Status tracks the active item continuously (including overtime): before the boundary it shows time left; after the boundary it switches to "PAST THE END OF [item]" while keeping the next-item line. Its number ruler uses whole-minute ticks in minute mode and tenths-of-an-hour ticks in hour mode; labels translate continuously and interpolate in size and opacity around the exact remaining value, including in the pop-out.
