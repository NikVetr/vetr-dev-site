...
## Details of the printed face

What the solver does inside one face once the columns are decided: the focused
card, the furniture around it, the pronunciation key, and the rule that a section
takes one shape at a time. These sat under "Themes" for a long time because that is
where they were first written down; they are layout, and they live beside `Pages`.
...
## Themes, scripts and accessibility
