...
Prefer fast, local checks over expensive full runs unless the latter are requested or warranted (eg after a major refactor). Try to keep the working tree clean.

For host-context commands that require shell features such as redirection, environment-variable assignments, command chaining, globbing, or multiple commands, always execute the complete operation explicitly through:

    /usr/bin/bash -lc '...'

Do not submit complex shell expressions directly for host-context execution. For simple commands that do not require shell syntax, invoke the executable
directly.
...
- PYTHONDONTWRITEBYTECODE=1 is supplied by the Codex environment; do not prefix Python commands with it.
...
- add large data folders (like data/, results/, most output/, and so on) to .gitignore so that they are not tracked, following git best practice
...
- avoid writing duplicated functions if a function already exists with the required (or highly similar) functionality
...
- minimize bloat and overly verbose or overly abstracted code. Aim for concise, readable code, covering plausible or important edge cases but not every possible edge case.
- write code that fails loudly. To repeat: write code that FAILS LOUDLY. Do not implement silent fallbacks that allow code to run or tests to pass while deviating from the design specification.
...
##  bash Guidelines
- For environment variables on executed commands, prefer:
    env NAME=value command ...
rather than:
    NAME=value command ...
- Prefer separate command executions over compound `&&`, `||`, `;`, or pipe expressions when practical.
- Prefer direct executable invocation over `bash -lc` / `zsh -lc` when shell features are not required.
