...
const out = [
  '# Which country each IANA timezone is in. Generated by scripts/build_timezones.mjs',
  `# from ${path} -- public domain, and the reference copy of this mapping.`,
  `# ${version.replace(/^#\s*/, '')}`,
  '# A zone spanning a border takes its most populous country, which is IANA\'s own',
  '# ordering. Region to language is the regions column of data/registry/languages.csv.',
  'timezone,iso3166',
...
if (process.argv.includes('--check')) {
  ...
  console.log(`data/registry/timezones.csv  ${rows.length} zones${changed ? '' : ' (unchanged)'}`);
}
