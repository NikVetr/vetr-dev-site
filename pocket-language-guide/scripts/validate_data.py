...
def main():
    ...
    # **The redundancy table has to match what it came from, and say which that is.**
    ...
    # the card. A row gets there one of two ways and `provenance` names which:
    #
    #   * `rated` -- aggregated from the independent passes in the file beside it, so
    #     a row edited here by hand is a judgement nobody made.
    #   * `rule:<name>` -- asserted by a named rule in `build_redundancy.RULES`,
    #     which is how `numbers-money` is modelled: 36% of the candidate pairs, and
    #     mechanical enough that a rule is more defensible than 12,000 judgements.
    #
    # Both halves are regenerated by `python3 scripts/build_redundancy.py`, so the
    # check is the same either way -- rebuild and compare. **What must not be shared
    # is the pass count.** An asserted row carries none, because none were made;
    # writing `passes,agree = 3,3` beside a rule would be the one thing this gate
    # exists to catch, a rating that no rater produced.
    ...
    if unknown:
        ...
        redundancy_concepts = build_redundancy.load_concepts()
        expected = build_redundancy.build_table(
            redundancy_concepts, build_redundancy.aggregate(pass_rows),
            build_redundancy.derive_asserted(redundancy_concepts))
        if [dict(r) for r in load("registry/redundancy.csv")] != expected:
            errors.append("registry/redundancy.csv does not match "
                          f"{rel} and build_redundancy.RULES "
                          "-- run `python3 scripts/build_redundancy.py`")

    provenances = {"rated"} | {f"rule:{name}" for name, _ in build_redundancy.RULES}
    seen = set()
    asserted_rows = 0
    for n, row in enumerate(load("registry/redundancy.csv"), start=2):
        ...
        if row["provenance"] not in provenances:
            errors.append(f"{where}: provenance {row['provenance']!r} is neither "
                          "`rated` nor a rule build_redundancy.RULES defines")
        elif row["provenance"] == "rated":
            if int(row["passes"] or 0) < 2:
                errors.append(f"{where}: rated by {row['passes']} pass(es); one judge "
                              "is one judge, so a relation needs at least two")
            if int(row["agree"] or 0) > int(row["passes"] or 0):
                errors.append(f"{where}: {row['agree']} of {row['passes']} passes agreed")
        else:
            asserted_rows += 1
            if row["passes"] or row["agree"]:
                errors.append(f"{where}: {row['provenance']} is a rule, but the row "
                              f"claims {row['passes']} pass(es) -- an asserted "
                              "relation has no raters to count")
    ...
    print(f"data/registry/redundancy.csv  {len(seen)} relations "
          f"({len(seen) - asserted_rows} rated, {asserted_rows} asserted by rule) "
          f"from {rated_pairs} rated pairs of "
