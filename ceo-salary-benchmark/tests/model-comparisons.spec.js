...
test("CV sorting uses numeric values and driver comparisons retain the focused model", async ({ page }) => {
  ...
  await expect(dialog.getByLabel("Compare all models")).toHaveCount(0);
  await expect(dialog.locator(".model-effect-row.is-selected")).toHaveCount(1);
  await expect(dialog.locator(".model-effect-row.is-selected .model-effect-selection")).toHaveCount(1);
  await expect(dialog.locator(".model-effect-row.is-selected .model-effect-interval-outline")).toHaveCount(1);
  ...
  await expect(dialog.locator(".model-effect-row.is-selected")).toHaveCount(levels.length + 1);
  await expect(dialog.locator(".model-effect-row.is-selected .model-effect-interval-outline")).toHaveCount(levels.length + 1);
  expect(await dialog.locator(".model-effect-row.is-selected").evaluateAll((rows) => rows.every((row) => row.dataset.modelKey === "bayesian"))).toBe(true);
  expect(errors).toEqual([]);
});
