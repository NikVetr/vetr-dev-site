...
def load_predictive_model_artifact(
    ...
    if set((artifact.get("models") or {})) != {
        ...
        "gamCategorical", "gamCategoricalNoHighest",
    }:
    ...
    for row in comparison:
        ...
        expected_method = ("bayesianExact" if row["key"].startswith("bayesian_exact") else
                           "bayesianGam" if row["key"].startswith("bayesian_gam") else
                           "gamCategorical" if row["key"].startswith("gam_categorical") else parts[0])
    ...
    gam_specifications = {"gam": True, "gamNoHighest": False, "gamCategorical": True, "gamCategoricalNoHighest": False}
    for model_key, include_highest in gam_specifications.items():
    ...
    categorical_provenance = artifact.get("categoricalGamProvenance") or {}
    for key, filename in {"fitScriptSha256": "fit_categorical_gam.R", "modelScriptSha256": "categorical_gam.R"}.items():
        if categorical_provenance.get(key) != hashlib.sha256((model_dir / filename).read_bytes()).hexdigest():
            raise ValueError(f"Categorical GAM artifact was not generated by the current {filename}")
    if categorical_provenance.get("trainingCsvSha256") != provenance["trainingCsvSha256"]:
        raise ValueError("Categorical GAM training data differ from the production cohort")
    levels_by_category = {item["key"]: item["levels"] for item in artifact["categoricalFeatures"]}
    levels_by_category["ea_relationship"] = artifact["eaLevels"]
    for key in ("gamCategorical", "gamCategoricalNoHighest"):
        model = artifact["models"][key]
        categories = model.get("categoryEffects") or {}
        if set(categories) != set(levels_by_category):
            raise ValueError(f"{key} does not use the full categorical taxonomy")
        width = len(model["uncertainty"]["coefficientDraws"][0])
        for name, levels in levels_by_category.items():
            effect = categories[name]
            if effect.get("levels") != levels:
                raise ValueError(f"{key} {name} category vocabulary differs")
            require_finite_vector(effect.get("values"), len(levels), f"{key} {name} effects")
            require_finite_matrix(effect.get("basis"), len(levels), width, f"{key} {name} basis")
            centered = effect["values"][0] if name == "ea_relationship" else sum(effect["values"])
            if abs(centered) > 1e-8:
                raise ValueError(f"{key} {name} category reference is not centered")
