...
Five PDFs (`S008`, `S036`, `S090`, `S093`, and `S112`) are image-only and remain unsuitable for numeric extraction until OCR is manually verified against the page images. Three manifest mappings (`S060`, `S082`, and duplicate mapping `S111`) point to two incomplete client-rendered HTML captures and are explicitly marked incomplete rather than treated as evidence. The last three regulator-controlled targets were retired as user-deprioritized on 2026-08-30: Empower Learning Africa Limited would require a paid authenticated Kenyan BRS Official Search, Animal Empathy Philippines, Inc. would require an interactive SEC Express request, and ALTER's entity-specific GuideStar Israel PDF is protected by an invisible reCAPTCHA. Their exact names and discovery URLs remain in `benchmark/enrichment/lingering_org_remaining_manual_save_requests.csv` only as an audit trail, not as active acquisition requests. GuideStar's title-level remuneration remains insufficient for a named observation without a direct person match.
