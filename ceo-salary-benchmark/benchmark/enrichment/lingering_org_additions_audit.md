...
Five PDFs (`S008`, `S036`, `S090`, `S093`, and `S112`) are image-only and remain unsuitable for numeric extraction until OCR is manually verified against the page images. Three manifest mappings (`S060`, `S082`, and duplicate mapping `S111`) point to two incomplete client-rendered HTML captures and are explicitly marked incomplete rather than treated as evidence. Three regulator-controlled acquisitions remain. Empower Learning Africa Limited requires a paid authenticated Kenyan BRS Official Search, and Animal Empathy Philippines, Inc. requires an interactive SEC Express request because neither registry exposes a stable entity-specific public filing URL. ALTER's entity-specific GuideStar Israel profile, 2024 finance/people pages, and exact 2024 report viewer are now identified, but the actual PDF download is protected by an invisible reCAPTCHA. The generic landing pages previously supplied as manual-save links were not evidence artifacts; the corrected statuses, exact organization names, and actionable URLs are preserved in `benchmark/enrichment/lingering_org_remaining_manual_save_requests.csv`. GuideStar's title-level remuneration remains insufficient for a named observation without a direct person match.
