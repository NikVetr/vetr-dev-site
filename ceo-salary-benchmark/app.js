(() => {
  ...
  if (!PREDICTIVE_MODEL || PREDICTIVE_MODEL.schemaVersion !== 3 || PREDICTIVE_MODEL.training?.rpExcluded !== true) {
  ...
  const state = {
    ...
    modelCompatibilityLevel: 89, modelEffectUnits: "percent", modelFocusWeights: null, modelTarget: "predictive",
  ...
  const PANEL_MINIMUMS = Object.freeze({
    ...
    chart: 368, results: 112, shortChart: 247, shortCompactChart: 283, shortResults: 60,
  });
  ...
  const refs = {
    ...
    stream: $("#stream-select"), streamField: $("#stream-field"), streamDescription: $("#stream-description"),
    measure: $("#measure-select"), measureField: $("#measure-field"),
    dollarBasis: [...document.querySelectorAll('input[name="dollar-basis"]')], dollarBasisField: $("#dollar-basis-field"),
    priceBasisStatus: $("#price-basis-status"), sample: $("#sample-select"), sampleField: $("#sample-field"),
    fit: [...document.querySelectorAll('input[name="distribution"]')], distributionField: $("#distribution-field"),
    ...
    chartViewTabs: [...document.querySelectorAll('.chart-view-tab[data-chart-view]')],
    chartViewTablist: $("#chart-view-tabs"), chartViewContent: $("#chart-view-content"),
    axisModes: {
    ...
    axisSettings: $("#axis-settings"), histogramAxisSettings: $("#histogram-axis-settings"), scatterAxisSettings: $("#scatter-axis-settings"),
    scatterControls: $("#scatter-controls"), scatterDisplaySettings: $("#scatter-display-settings"),
    contourField: $("#contour-field"), jobAdIntervalField: $("#job-ad-interval-field"),
    chartColor: $("#chart-color"), chartColorField: $("#chart-color-field"), colorDescription: $("#color-description"), showContours: $("#show-contours"),
    weightingField: $("#weighting-field"),
    ...
    modelTrainingCount: $("#model-training-count"),
    modelMethodDescription: $("#model-method-description"), modelComparisonBody: $("#model-comparison-body"),
    modelContributions: $("#model-contributions"), modelLimitations: $("#model-limitations"),
    ...
    statN: $("#stat-n"), statNUnit: $("#stat-n-unit"), statNeff: $("#stat-neff"),
    statNeffUnit: $("#stat-neff-unit"), statCenter: $("#stat-center"), statCenterUnit: $("#stat-center-unit"),
  ...
  function rows() {
    ...
    if (state.view === "model") return [...incumbents, ...jobAds];
  ...
  function axisUsesSalary(axisKey) {
    const expression = axisExpression(axisKey);
    return expression.numerator === "salary"
      || (axisMode(axisKey) === "ratio" && expression.denominator === "salary");
  }

  function axisValueAtSalary(axisKey, row, salaryValue) {
    const expression = axisExpression(axisKey);
    const valueFor = (key) => key === "salary" ? salaryValue : numericVariables[key]?.value(row);
    const numerator = valueFor(expression.numerator);
    if (axisMode(axisKey) === "value") return numerator;
    const denominator = valueFor(expression.denominator);
    return Number.isFinite(numerator) && Number.isFinite(denominator) && numerator > 0 && denominator > 0
      ? numerator / denominator : null;
  }

  function axisInterval(axisKey, row) {
    if (!state.showJobAdIntervals || !axisUsesSalary(axisKey)) return null;
    const range = salaryRange(row);
    if (!(range?.low > 0) || !(range?.high > range.low)) return null;
    const first = axisValueAtSalary(axisKey, row, range.low);
    const second = axisValueAtSalary(axisKey, row, range.high);
    if (!Number.isFinite(first) || !Number.isFinite(second) || first <= 0 || second <= 0) return null;
    return { low: Math.min(first, second), high: Math.max(first, second), atLowSalary: first, atHighSalary: second };
  }
  ...
  function plotEligibility(row) {
    if (state.view === "model") {
      ...
      if (["gam", "linear", "intercept", "svr", "gp"].includes(state.modelMethod) && trainingRecord.observation !== "exact_base") {
        return {
          eligible: false,
          reason: `${currentModelComparisonRow().label} uses exact Form 990 base salaries only.`,
        };
      }
  ...
  function analysisAxisKey() { return state.view === "scatter" ? "scatterY" : state.view === "histogram" ? "histogram" : null; }
  function analysisItems() { return state.view === "model" ? [] : state.view === "scatter" ? currentPlotItems() : axisItems("histogram"); }
  ...
  function presetSelected(row) {
    ...
    if (state.sample === "sensitivity") return Boolean(available && (
      row.defaultIncluded || ["sensitivity_only", "structural_sensitivity", "sensitivity_hours_review"].includes(row.analysisStatus)
    ));
  ...
  function renderScatterLegend(colorMap, contoursShown, items, intervalsShown = false) {
    refs.chartLegend.replaceChildren();
    appendCategoryLegend(colorMap);
    if (intervalsShown) {
      const interval = document.createElement("span");
      interval.innerHTML = '<i class="interval-swatch"></i> Advertised range · point is midpoint';
      refs.chartLegend.append(interval);
    }
    if (contoursShown) {
  ...
  function renderScatter() {
    ...
    const advertisedIntervals = items.map((item) => ({
      item,
      x: axisInterval("scatterX", item.row),
      y: axisInterval("scatterY", item.row),
    })).filter((interval) => interval.x || interval.y);
    if (advertisedIntervals.length) {
      $("#chart-description").textContent += " Advertised low-to-high ranges are drawn around job-posting midpoints.";
    }
    const advertisedIntervalsById = new Map(advertisedIntervals.map((interval) => [interval.item.row.id, interval]));
    advertisedIntervals.forEach((interval) => {
      if (interval.x) transformedX.push(xTransform(interval.x.low), xTransform(interval.x.high));
      if (interval.y) transformedY.push(yTransform(interval.y.low), yTransform(interval.y.high));
    });
    rpItems.forEach((item) => {
    ...
    const intervalGroup = svgElement("g", { class: "job-ad-intervals", "clip-path": `url(#${clipId})`, "aria-hidden": "true" });
    advertisedIntervals.forEach(({ item, x: xInterval, y: yInterval }) => {
      const midpointX = xScale(item.xValue);
      const midpointY = yScale(item.yValue);
      if (xInterval && yInterval) {
        const range = salaryRange(item.row);
        const locus = Array.from({ length: 25 }, (_, index) => {
          const salaryValue = range.low + index / 24 * (range.high - range.low);
          return {
            x: axisValueAtSalary("scatterX", item.row, salaryValue),
            y: axisValueAtSalary("scatterY", item.row, salaryValue),
          };
        }).filter((point) => Number.isFinite(point.x) && Number.isFinite(point.y));
        if (locus.length > 1) {
          const path = locus.map((point, index) => `${index ? "L" : "M"}${xScale(point.x).toFixed(2)},${yScale(point.y).toFixed(2)}`).join(" ");
          intervalGroup.append(svgElement("path", { d: path, class: "job-ad-interval" }));
          [locus[0], locus.at(-1)].forEach((point) => {
            intervalGroup.append(svgElement("circle", {
              cx: xScale(point.x), cy: yScale(point.y), r: 2.2,
              class: "job-ad-interval job-ad-interval-endpoint",
            }));
          });
        }
        return;
      ...
      if (xInterval) {
        const x1 = xScale(xInterval.low); const x2 = xScale(xInterval.high);
        intervalGroup.append(svgElement("line", { x1, x2, y1: midpointY, y2: midpointY, class: "job-ad-interval" }));
        intervalGroup.append(svgElement("line", { x1, x2: x1, y1: midpointY - 3, y2: midpointY + 3, class: "job-ad-interval job-ad-interval-cap" }));
        intervalGroup.append(svgElement("line", { x1: x2, x2, y1: midpointY - 3, y2: midpointY + 3, class: "job-ad-interval job-ad-interval-cap" }));
      }
      if (yInterval) {
        const y1 = yScale(yInterval.low); const y2 = yScale(yInterval.high);
        intervalGroup.append(svgElement("line", { x1: midpointX, x2: midpointX, y1, y2, class: "job-ad-interval" }));
        intervalGroup.append(svgElement("line", { x1: midpointX - 3, x2: midpointX + 3, y1, y2: y1, class: "job-ad-interval job-ad-interval-cap" }));
        intervalGroup.append(svgElement("line", { x1: midpointX - 3, x2: midpointX + 3, y1: y2, y2, class: "job-ad-interval job-ad-interval-cap" }));
      }
    });
    svg.append(intervalGroup);
    ...
    points.forEach(({ item, x, y }) => {
      const category = chartCategory(item.row);
      const advertisedInterval = advertisedIntervalsById.get(item.row.id);
      const advertisedRangeLabel = [
        advertisedInterval?.y
          ? `${yDescriptor.label} advertised range ${yDescriptor.fullFormat(advertisedInterval.y.low)} to ${yDescriptor.fullFormat(advertisedInterval.y.high)}` : "",
        advertisedInterval?.x
          ? `${xDescriptor.label} advertised range ${xDescriptor.fullFormat(advertisedInterval.x.low)} to ${xDescriptor.fullFormat(advertisedInterval.x.high)}` : "",
      ].filter(Boolean).join(", ");
      ...
      const point = svgElement("circle", {
        ...
        "aria-label": [
          item.row.organization, item.row.title || "reported role",
          `${yDescriptor.label}${advertisedInterval?.y ? " midpoint" : ""} ${yDescriptor.fullFormat(item.yValue)}`,
          `${xDescriptor.label}${advertisedInterval?.x ? " midpoint" : ""} ${xDescriptor.fullFormat(item.xValue)}`,
          advertisedRangeLabel,
        ].filter(Boolean).join(", "),
      });
      ...
      const showPointTooltip = (event) => {
        if (Number.isFinite(event.clientX) && Number.isFinite(event.clientY) && (event.clientX || event.clientY)) {
          showTooltip(event, item, tooltipContext);
          return;
        }
        const bounds = point.getBoundingClientRect();
        showTooltip({ clientX: bounds.left + bounds.width / 2, clientY: bounds.top + bounds.height / 2 }, item, tooltipContext);
      };
      point.addEventListener("pointerenter", showPointTooltip);
      point.addEventListener("pointermove", positionTooltip); point.addEventListener("pointerleave", hideTooltip);
      point.addEventListener("focus", showPointTooltip); point.addEventListener("blur", hideTooltip);
    ...
    renderScatterLegend(colors, contoursShown, items, advertisedIntervals.length > 0);
    const totalWeight = items.reduce((sum, item) => sum + item.weight, 0);
    const squared = items.reduce((sum, item) => sum + item.weight ** 2, 0);
    refs.statN.textContent = items.length;
    refs.statNeff.textContent = squared ? (totalWeight ** 2 / squared).toFixed(1) : "0";
  ...
  function activeModelKey() {
    ...
    if (isBayesianMethod() && state.modelMethod !== "bayesianExact" && state.modelUseAdRanges) return `${state.modelMethod}Ranges${suffix}`;
  ...
  function modelCategoryDefinition(key) {
    return PREDICTIVE_MODEL.categoricalFeatures.find((feature) => feature.key === key);
  }

  function modelCategoryIndex(key, value) {
    if (value === "__average__" || (key === "focus_area" && value === "__mixture__")) return -1;
    const index = modelCategoryDefinition(key)?.levels.indexOf(value) ?? -1;
    if (index < 0) throw new Error(`Unknown predictive-model category: ${key}=${value}`);
    return index;
  }

  function median(values) { return sampleQuantile(values, 0.5); }

  function medianAcrossDraws(values) {
    return median(values.filter(Number.isFinite));
  }

  function interpolateModelEffect(effect, value) {
    const x = effect.z; const y = effect.effect;
    if (!x.length || x.length !== y.length) throw new Error("Invalid GAM effect grid");
    if (value <= x[0]) return y[0];
    if (value >= x.at(-1)) return y.at(-1);
    let low = 0; let high = x.length - 1;
    while (high - low > 1) {
      const middle = Math.floor((low + high) / 2);
      if (x[middle] <= value) low = middle; else high = middle;
    }
    const fraction = (value - x[low]) / (x[high] - x[low]);
  ...
  function gamModelPrediction() {
    ...
    return residualCalibratedPrediction(currentModelComparisonRow()?.key || "gam", model, mu, contributions, muDraws);
  }

  function residualCalibratedPrediction(methodKey, model, mu, contributions = [], muDraws = null) {
    ...
    return {
      methodKey, model, mu, muDraws, distribution, reference: mu - contributions.reduce((sum, item) => sum + item.value, 0),
  ...
  function interceptModelPrediction() {
    ...
    return residualCalibratedPrediction("intercept", model, model.baseline, [], model.uncertainty.coefficientDraws.map((draw) => draw[0]));
  }
  ...
  function linearModelPrediction() {
    ...
    const contributions = continuousKeys.map((key, index) => ({
      ...
      draws: model.uncertainty.coefficientDraws.map((draw) => {
        const column = designColumns.indexOf(`log_${key}`);
        return column >= 0 ? standardized[index] * draw[column + 1] : 0;
      }),
    }));
    ...
    return residualCalibratedPrediction(currentModelComparisonRow()?.key || "linear", model, mu, contributions,
      model.uncertainty.coefficientDraws.map((draw) => SalaryModelMath.dot([1, ...vector], draw)));
  }
  ...
  function bayesianModelPrediction() {
    ...
    const focusWeights = state.modelProfile.focus_area === "__mixture__" ? state.modelFocusWeights : null;
    if (state.modelProfile.focus_area === "__mixture__" && (!Array.isArray(focusWeights)
      || focusWeights.length !== draws.focus[0].length || focusWeights.some((value) => !Number.isFinite(value) || value < 0)
      || Math.abs(focusWeights.reduce((sum, value) => sum + value, 0) - 100) > .01)) return null;
    const curve = (column, value, draw) => model.curvature
      ? SalaryModelMath.dot(SalaryModelMath.curvature(value, model.curvature[column]), draws.smooth[column][draw]) : 0;
    ...
    for (let index = 0; index < draws.alpha.length; index += 1) {
      ...
      value += continuousKeys.reduce((sum, key, column) => sum + curve(column, vector[column], index), 0);
      Object.entries(categoryIndexes).forEach(([key, categoryIndex]) => {
        if (categoryIndex >= 0) value += draws[key][index][categoryIndex];
      });
      if (focusWeights) value += SalaryModelMath.dot(focusWeights, draws.focus[index]) / 100;
      mu.push(value);
    }
    const contributionSpecs = [
      ...continuousKeys.map((key, column) => [MODEL_CONTINUOUS_LABELS[key], () => draws.beta.map((row, i) => row[column] * vector[column]
        + curve(column, vector[column], i) - curve(column, 0, i)), key]),
      ["Focus area", () => focusWeights ? draws.focus.map((row) => SalaryModelMath.dot(focusWeights, row) / 100)
        : categoryIndexes.focus < 0 ? [0] : draws.focus.map((row) => row[categoryIndexes.focus]), "focus"],
      ...
      ["CEO hiring market", () => categoryIndexes.location < 0 ? [0] : draws.location.map((row) => row[categoryIndexes.location])],
    ...
    return {
      ...
      reference: medianAcrossDraws(draws.alpha.map((alpha, i) => alpha + continuousKeys.reduce((sum, key, j) => sum + curve(j, 0, i), 0))),
      expected: distribution.expected,
      median: distribution.quantile(0.5),
      contributions: contributionSpecs.map(([label, accessor, key]) => ({ label, key, draws: accessor(), value: medianAcrossDraws(accessor()) })),
    };
  }

  function kernelModelPrediction() {
    const model = activeModelArtifact();
    const vector = modelContinuousVector(model);
    if (vector.some((value) => !Number.isFinite(value))) return null;
    const family = state.modelMethod;
    const result = SalaryModelMath.kernelPrediction(model.kernel, vector, family);
    const contributions = SalaryModelMath.kernelContributions(model.kernel, vector, family, model.uncertainty.kernelDraws)
      .map((item, j) => ({ ...item, key: modelContinuousKeys(model)[j], label: MODEL_CONTINUOUS_LABELS[modelContinuousKeys(model)[j]] }));
    if (family === "svr") return residualCalibratedPrediction(currentModelComparisonRow().key, model, result.mean, contributions,
      model.uncertainty.kernelDraws.map((draw) => SalaryModelMath.kernelPrediction(model.kernel, vector, family, draw).mean));
    const distribution = logNormalMixture([result.mean], [Math.sqrt(result.variance + model.kernel.noise ** 2)]);
    return { methodKey: currentModelComparisonRow().key, model, mu: result.mean, latentVariance: result.variance,
      distribution, expected: distribution.expected, median: distribution.quantile(.5), contributions,
      reference: SalaryModelMath.kernelPrediction(model.kernel, vector.map(() => 0), family).mean };
  }

  const modelPredictionCache = new Map();
  function expectationView() { return state.modelTarget === "expectation"; }

  function locationDistribution(prediction) {
    if (state.modelMethod === "gp") return logNormalMixture([prediction.mu], [Math.sqrt(prediction.latentVariance)]);
    const draws = Array.isArray(prediction.mu) ? prediction.mu : prediction.muDraws;
    if (!draws?.length || draws.some((value) => !Number.isFinite(value))) throw new Error("Missing finite log-location draws");
    const mean = draws.reduce((sum, value) => sum + value, 0) / draws.length;
    const sd = Math.sqrt(draws.reduce((sum, value) => sum + (value - mean) ** 2, 0) / (draws.length - 1));
    // Smooth only the plotted density; summaries use the retained draws directly.
    const distribution = logNormalMixture(draws, draws.map(() => Math.max(1e-6, 1.06 * sd * draws.length ** -.2)));
    return { ...distribution, expected: draws.reduce((sum, value) => sum + Math.exp(value), 0) / draws.length,
      quantile: (p) => Math.exp(sampleQuantile(draws, p)),
      cdf: (value) => draws.filter((mu) => mu <= Math.log(value)).length / draws.length };
  }

  function currentModelPrediction() {
    if (!isCeoPosition()) return null;
    const key = JSON.stringify([activeModelKey(), state.modelProfile, state.modelFocusWeights, state.modelTarget]);
    ...
    if (prediction) {
      ...
      if (expectationView()) {
        prediction.distribution = locationDistribution(prediction);
        prediction.expected = prediction.distribution.expected;
        prediction.median = prediction.distribution.quantile(.5);
      }
    }
    if (modelPredictionCache.size >= 64) modelPredictionCache.clear();
    modelPredictionCache.set(key, prediction);
    return prediction;
  }

  // Prediction comparisons are synchronous and never change controls or the saved profile.
  function withModelComparison(row, evaluate) {
    const saved = [state.modelMethod, state.modelUseAdRanges, state.modelIncludeHighestOtherPay];
    const spec = modelComparisonSpec(row);
    try {
      state.modelMethod = spec.method;
      state.modelUseAdRanges = spec.useAdRanges;
      state.modelIncludeHighestOtherPay = spec.includeHighestOtherPay;
      return evaluate();
    } finally {
      [state.modelMethod, state.modelUseAdRanges, state.modelIncludeHighestOtherPay] = saved;
    }
  }
  ...
  function renderProfileResults() {
    renderChart();
    renderQuantiles();
    if (state.view === "model" && activeResultsTab === "robustness") runModelRobustnessAnalysis();
  }

  function quantileUncertainty(prediction, probability) {
    const level = state.modelCompatibilityLevel / 100;
    const key = `${probability}:${level}`;
    if (prediction.quantileUncertaintyCache.has(key)) return prediction.quantileUncertaintyCache.get(key);
    const tail = (1 - level) / 2;
    const z = SalaryModelMath.normalQuantile(probability);
    let interval;
    if (expectationView()) {
      interval = [prediction.distribution.quantile(tail), prediction.distribution.quantile(1 - tail)];
    } else if (isBayesianMethod()) {
      const quantiles = prediction.mu.map((mu, i) => Math.exp(mu + prediction.model.draws.sigma[i][0] * z));
      interval = [sampleQuantile(quantiles, tail), sampleQuantile(quantiles, 1 - tail)];
    } else if (state.modelMethod === "gp") {
      const center = prediction.mu + prediction.model.kernel.noise * z;
      const radius = SalaryModelMath.normalQuantile(1 - tail) * Math.sqrt(prediction.latentVariance);
      interval = [Math.exp(center - radius), Math.exp(center + radius)];
    } else {
      const quantiles = prediction.muDraws.map((mu, i) => {
        const residuals = prediction.model.uncertainty.residualDraws[i];
        const mean = residuals.reduce((sum, value) => sum + value, 0) / residuals.length;
        ...
        return logNormalMixture(residuals.map((value) => value + mu), residuals.map(() => bandwidth)).quantile(probability);
      });
      interval = [sampleQuantile(quantiles, tail), sampleQuantile(quantiles, 1 - tail)];
    }
    prediction.quantileUncertaintyCache.set(key, interval);
    return interval;
  }
  ...
  function modelComparisonSpec(row) {
    if (!row) return null;
    if (!row.method || !PREDICTIVE_MODEL.models[row.modelKey]) throw new Error("Invalid model comparison registry");
    return { method: row.method, useAdRanges: row.includeAdvertisedRanges,
      includeHighestOtherPay: row.method === "intercept" ? state.modelIncludeHighestOtherPay : row.includeHighestOtherPay };
  }
  ...
  function currentModelComparisonRow() {
    return PREDICTIVE_MODEL.comparison.find((row) => {
      ...
      return spec?.method === state.modelMethod
        && (!isBayesianMethod() || spec.useAdRanges === state.modelUseAdRanges)
  ...
  function activeModelTrainingRecords() {
    ...
    return PREDICTIVE_MODEL.training.records.filter((record) => {
      if (["gam", "linear", "intercept", "svr", "gp"].includes(state.modelMethod)) return record.observation === "exact_base";
  ...
  function renderModelContributions(prediction) {
    ...
    const effectValue = (value) => state.modelEffectUnits === "percent" ? Math.expm1(value) * 100 : value;
    const maximum = Math.max(...prediction.contributions.map((item) => Math.abs(effectValue(item.value))), 0.01);
    $("#model-driver-reference").textContent = `Reference: ${modelMoney(Math.exp(prediction.reference))}. Click a driver for uncertainty.`;
    const heading = resultElement("div", "model-contribution-heading");
    heading.append(resultElement("span", "", "Driver"), resultElement("span", "", state.modelEffectUnits === "percent" ? "Effect (%)" : "Effect (log)"));
    refs.modelContributions.append(heading);
    prediction.contributions.forEach((item) => {
      const row = document.createElement("div");
      row.className = `model-contribution${item.value < 0 ? " is-negative" : ""}`;
      const label = document.createElement("button"); label.type = "button"; label.className = "model-driver-button"; label.textContent = item.label;
      label.setAttribute("aria-label", `Inspect ${item.label} uncertainty`);
      label.addEventListener("click", () => showDriverExplanation(prediction, item));
      const track = document.createElement("span"); track.className = "model-contribution-track";
      const bar = document.createElement("i"); bar.className = "model-contribution-bar";
      const width = Math.max(0, Math.abs(effectValue(item.value)) / maximum * 50);
      bar.style.width = `${width}%`;
      bar.style.left = item.value < 0 ? `${50 - width}%` : "50%";
      track.append(bar);
      const value = document.createElement("output"); value.textContent = formatModelEffect(item.value);
      value.style.left = `calc(${item.value < 0 ? 50 : 50 + width}% + 5px)`;
      track.append(value);
  ...
  function formatModelEffect(value) {
    const transformed = state.modelEffectUnits === "percent" ? 100 * Math.expm1(value) : value;
    return `${transformed >= 0 ? "+" : ""}${transformed.toFixed(state.modelEffectUnits === "percent" ? 1 : 3)}${state.modelEffectUnits === "percent" ? "%" : ""}`;
  }

  function showModelSpecification(row) {
    $("#model-explanation-title").textContent = row.label;
    $("#model-explanation-content").innerHTML = salaryModelVignette(row, PREDICTIVE_MODEL.models[row.modelKey], PREDICTIVE_MODEL);
  ...
  function showDriverExplanation(prediction, item) {
    ...
    const categorySpecs = {
      ...
      "CEO hiring market": ["location", "location_scope"], "Work model": ["remote", "remote_category"],
      "Fiscal sponsor": ["fiscalSponsor", "fiscal_sponsor_category"],
    };
    const spec = categorySpecs[item.label];
    const tail = (1 - state.modelCompatibilityLevel / 100) / 2;
    const summary = (label, draws, normal = null) => ({ label,
      center: normal ? normal.mean : sampleQuantile(draws, .5),
  ...
  function selectModelComparisonMethod(methodKey, { focus = true } = {}) {
    ...
    requestAnimationFrame(() => {
      if (focus) refs.modelComparisonBody.querySelector(`[data-model-method="${methodKey}"]`)?.focus();
    });
  }

  let modelComparisonSort = { key: "", direction: 1 };
  document.querySelectorAll("[data-model-sort]").forEach((button) => button.addEventListener("click", () => {
    const key = button.dataset.modelSort;
    modelComparisonSort = { key, direction: key === modelComparisonSort.key ? -modelComparisonSort.direction : 1 };
  ...
  function renderModelDiagnostics(prediction) {
    ...
    refs.modelCvError.textContent = `${(metric.meanAbsPercentError * 100).toFixed(0)}%`;
    refs.modelCvCoverage.textContent = `${(metric.coverage90 * 100).toFixed(0)}%`;
    refs.modelProfileSupport.textContent = coverage.label;
    refs.modelProfileSupport.classList.toggle("model-profile-warning", coverage.warnings.length > 0);
    refs.modelProfileSupport.title = coverage.warnings.join("; ");
    refs.modelProfileSupport.setAttribute("aria-describedby", "model-profile-reasons");
    ...
    refs.modelMethodDescription.innerHTML = "<strong>Same held-out base salaries and grouped folds.</strong> Click a column to sort, a method to select, or (?) for its specification.";
    refs.modelComparisonBody.replaceChildren();
    document.querySelectorAll("[data-model-sort]").forEach((button) => {
      button.closest("th").setAttribute("aria-sort", button.dataset.modelSort === modelComparisonSort.key
        ? modelComparisonSort.direction === 1 ? "ascending" : "descending" : "none");
    });
    const sortValue = (row) => modelComparisonSort.key === "method" ? modelFamilyLabel(row) : row[modelComparisonSort.key];
    const rows = [...PREDICTIVE_MODEL.comparison];
    if (modelComparisonSort.key) rows.sort((a, b) => modelComparisonSort.direction * (typeof sortValue(a) === "string"
      ? sortValue(a).localeCompare(sortValue(b)) : Number(sortValue(a)) - Number(sortValue(b))));
    rows.forEach((row) => {
      ...
      if (modelComparisonSpec(row)) {
        ...
        button.textContent = modelFamilyLabel(row);
  ...
  function renderModelLegend() {
    ...
    [["model-legend-50", "50% prediction interval"], ["model-legend-80", "80% prediction interval"], ["model-legend-95", "95% prediction interval"]].forEach(([className, label]) => {
      const item = document.createElement("span"); item.innerHTML = `<i class="swatch ${className}"></i> ${expectationView() ? label.replace("prediction", "estimation") : label}`; refs.chartLegend.append(item);
    });
  }
  ...
  function renderModel() {
    ...
    svg.setAttribute("viewBox", `0 0 ${width} ${height}`);
    refs.chartLegend.replaceChildren();
    if (!prediction || prediction.distribution.count < 1) {
      refs.modelDiagnostics.hidden = true;
      clearModelDetailsForInvalidPrediction();
      const empty = svgElement("text", { x: width / 2, y: height / 2, "text-anchor": "middle", fill: "#52879E", "font-size": 13 });
      empty.textContent = isCeoPosition() ? (isBayesianMethod() && state.modelProfile.focus_area === "__mixture__"
        ? "Enter positive numeric inputs and focus shares totaling 100%."
        : "Enter positive model inputs to generate a prediction.") : "The predictive model is currently validated only for CEO pay.";
      svg.append(empty);
      $("#chart-description").textContent = empty.textContent;
      refs.statN.textContent = "—"; refs.statNeff.textContent = "—"; refs.statCenter.textContent = "—";
      return;
    }
    ...
    if (geometry.transform(referenceSalary) >= axisMin && geometry.transform(referenceSalary) <= axisMax) {
      ...
      referenceMarker = appendRpMarker(svg, referenceX, rpLogoY, {
        ...
        rankDetails: [[expectationView() ? "Estimation percentile" : "Predictive percentile", `${predictivePercentile.toFixed(1)} (model distribution)`]],
      }, markerSize);
  ...
  function renderQuantiles() {
    if (state.view === "model") {
      ...
      if (state.quantileGranularity === "custom") {
        refs.customQuantilesError.textContent = percentiles.length
          ? "" : "Enter comma-separated values greater than 0 and less than 100.";
  ...
  function renderTable() {
    ...
    const selection = state.view === "model" ? [] : currentPlotItems();
    const weightMap = state.view === "model"
    ...
    tableRows(weightMap, eligibilityMap).forEach((row) => {
      ...
      toggle.checked = state.view === "model" ? eligibility.eligible : available && rowInclusion(row).get(row.id);
      toggle.disabled = !available || state.view === "model";
      ...
      const toggleHelp = state.view === "model"
        ? `${row.organization}: the model uses a fixed reviewed training cohort; table selections are preserved but do not refit it.`
        : inclusionHelp;
      toggle.setAttribute("aria-label", toggleHelp);
      toggle.title = toggleHelp;
      ...
      const location = document.createElement("td"); location.className = "metadata-cell";
      const market = document.createElement("button"); market.type = "button"; market.className = "metadata-evidence-button";
      market.textContent = row.ceoHiringMarket === "Location not reported" ? "NA" : tableDisplayLabel(row.ceoHiringMarket);
      market.title = `CEO hiring market: ${row.ceoHiringMarket}. Basis: ${humanizeCategory(row.ceoHiringMarketBasis)}.`;
      market.addEventListener("click", () => openOperatingMetadataDialog(row, "remote"));
      const footprint = document.createElement("small"); footprint.className = "organization-footprint";
      footprint.textContent = `Footprint: ${{ us: "US", international: "international", outside_us: "outside US", unknown: "unknown" }[row.operatingFootprint] || row.operatingFootprint}`;
      location.append(market, footprint);
  ...
  function plainAnalysisStatus(value) {
    ...
    const labels = {
      ...
      sensitivity_hours_review: "Sensitivity only; full-time role hours unresolved",
      included: "Included",
      no: "Not included",
    };
  ...
  function openSourceDialog(row) {
    ...
    [
      ...
      ["Source location / work arrangement", [row.sourceLocationDescription, row.remoteStatus].filter(Boolean).join(" · ") || "Not reported"],
      ...
      ...(row.eligibilityReview ? [["Role eligibility review", row.eligibilityReview]] : []),
  ...
  function openOperatingMetadataDialog(row, field) {
    ...
    if (remote) (metadata.reviewEvidence || []).forEach((item) => {
      const dt = document.createElement("dt"); dt.textContent = `${item.role || "Source"} · ${item.published_date || item.date || "Date unknown"}`;
  ...
  function updateHeadings() {
    ...
    refs.measureField.hidden = isModel || isJobs || isCombined;
    refs.streamField.hidden = isModel;
    refs.dollarBasisField.hidden = isModel;
    refs.sampleField.hidden = isModel;
    refs.distributionField.hidden = isModel;
    refs.axisSettings.hidden = isModel;
    refs.chartColorField.hidden = isModel;
    refs.weightingField.hidden = isModel;
    refs.settingsPanel.classList.toggle("is-model-view", isModel);
    ...
    if (isModel) {
      ...
      refs.statNeffUnit.textContent = "effective sample size";
    ...
    refs.chartViewTabs.forEach((tab) => {
      const selected = tab.dataset.chartView === state.view;
      tab.setAttribute("aria-selected", String(selected));
      tab.tabIndex = selected ? 0 : -1;
      if (tab.dataset.chartView === "model") {
        tab.setAttribute("aria-disabled", String(!isCeoPosition()));
        tab.setAttribute("aria-label", isCeoPosition()
          ? "Model"
          : "Model, unavailable because the predictive model is currently validated only for CEO pay");
        tab.title = isCeoPosition() ? "Predict salary from organization characteristics" : "The predictive model is currently validated only for CEO pay";
      }
    });
  ...
  function activateChartView(view, { focus = false } = {}) {
    if (!["histogram", "scatter", "model"].includes(view)) return;
    if (view === "model" && !isCeoPosition()) return;
    state.view = view;
    state.focusedId = "";
    ...
    renderAll();
    if (focus) refs.chartViewTabs.find((tab) => tab.dataset.chartView === view)?.focus();
  }

  function chartViewKeyFromKeyboard(event) {
    const available = refs.chartViewTabs.filter((tab) => tab.getAttribute("aria-disabled") !== "true");
    const current = available.indexOf(event.currentTarget);
    if (event.key === "Home") return available[0]?.dataset.chartView || "";
    if (event.key === "End") return available.at(-1)?.dataset.chartView || "";
  ...
  function modelCategoryLevels(key) {
    if (key === "ea_relationship") return PREDICTIVE_MODEL.eaLevels;
    return modelCategoryDefinition(key)?.levels || [];
  }

  function encodeModelCategory(key, value) {
    if (key === "focus_area" && value === "__mixture__") return -1;
    if (value === "__average__") return 0;
    const index = modelCategoryLevels(key).indexOf(value);
    if (index < 0) throw new Error(`Unknown predictive-model category: ${key}=${value}`);
    return index + 1;
  }

  function decodeModelCategory(key, code, fallback) {
    if (key === "focus_area" && code === -1) return "__mixture__";
  ...
  function compactModelState() {
    ...
    Object.entries(MODEL_PROFILE_URL_FIELDS).forEach(([key, code]) => {
      ...
      compact[code] = categorical
        ? encodeModelCategory(key, value) : value;
    });
    return compact;
  }
  ...
  function sharePayload() {
    if (state.view === "model") {
      const payload = { v: URL_STATE_VERSION, l: 2 };
      const modelState = compactModelState();
      if (Object.keys(modelState).length) payload.y = modelState;
  ...
  function activateResultsTab(key, { focus = false } = {}) {
    ...
    if (key === "robustness") {
      ...
      if (state.view === "model") expandResultsPanelForRobustness();
    }
  }
  ...
  function scenarioSummary() {
    if (state.view === "model") {
      ...
      return {
        ...
        formatted: {
          p25: compactMoney(values[0]), p50: compactMoney(values[1]), p75: compactMoney(values[2]),
          rpValue: compactMoney(reference),
        },
      };
    }
    const items = analysisItems();
  ...
  function validStoredScenario(value) {
    ...
    try {
      return SUPPORTED_URL_STATE_VERSIONS.includes(decodeUrlState(value.encodedState)?.v);
    } catch {
      return false;
    }
  }
  ...
  function presentationState() {
    return {
      ...
      showJobAdIntervals: state.showJobAdIntervals,
  ...
  function restorePresentationState(presentation) {
    ...
    state.showJobAdIntervals = presentation.showJobAdIntervals !== false;
  ...
  function benchmarkHistorySnapshot() {
    const stateSnapshot = structuredClone(state);
    stateSnapshot.weightings = [...state.weightings].sort((a, b) => a.localeCompare(b));
    stateSnapshot.filters = Object.fromEntries(Object.entries(state.filters).map(([key, values]) => [
      key,
      values == null ? null : [...values].map(String).sort((a, b) => a.localeCompare(b)),
    ]));
    delete stateSnapshot.focusedId;
    delete stateSnapshot.hoverQuantile;
    delete stateSnapshot.autoBins;
    const rowState = Object.fromEntries(Object.keys(allRowsByStream).map((stream) => [stream, {
      inclusion: allRowsByStream[stream].map((row) => [row.id, Boolean(inclusion[stream].get(row.id))]),
      customWeights: allRowsByStream[stream].map((row) => [row.id, customWeights[stream].get(row.id) ?? 1]),
      modifiedWeightIds: [...modifiedWeightIds[stream]].sort((a, b) => a.localeCompare(b)),
    }]));
    return {
      version: 1,
      state: stateSnapshot,
  ...
  function historyEntry(snapshot = benchmarkHistorySnapshot()) {
  ...
  function recordBenchmarkHistory(coalesceKey = "") {
    if (historyRestoring || !currentHistoryEntry) return;
    const nextEntry = historyEntry();
    if (nextEntry.signature === currentHistoryEntry.signature) return;
    const coalescing = Boolean(coalesceKey && coalesceKey === historyCoalesceKey);
    if (!coalescing) {
      undoHistory.push(currentHistoryEntry);
      if (undoHistory.length > HISTORY_LIMIT) undoHistory.shift();
    } else if (undoHistory.at(-1)?.signature === nextEntry.signature) {
      undoHistory.pop();
  ...
  function restoreBenchmarkHistorySnapshot(snapshot) {
    if (!snapshot || snapshot.version !== 1 || !snapshot.state || !snapshot.rowState) {
      throw new Error("Unsupported benchmark history state.");
    }
    const restoredState = structuredClone(snapshot.state);
    restoredState.weightings = new Set(restoredState.weightings || []);
    restoredState.filters = Object.fromEntries(Object.entries(restoredState.filters || {}).map(([key, values]) => [
      key,
      values == null ? null : new Set(values.map(String)),
    ]));
    restoredState.focusedId = "";
    restoredState.hoverQuantile = null;
    restoredState.autoBins = false;
    Object.assign(state, restoredState);
    Object.entries(snapshot.rowState).forEach(([stream, values]) => {
      if (!inclusion[stream] || !customWeights[stream] || !modifiedWeightIds[stream]) return;
      values.inclusion.forEach(([id, selected]) => {
        if (inclusion[stream].has(id)) inclusion[stream].set(id, Boolean(selected));
      });
      values.customWeights.forEach(([id, value]) => {
        if (customWeights[stream].has(id)) customWeights[stream].set(id, Number(value));
      });
      modifiedWeightIds[stream].clear();
      values.modifiedWeightIds.forEach((id) => {
    ...
    hideTooltip();
    hideRobustnessTooltip();
    refs.helpTooltip.hidden = true;
    refs.organizationPreview.hidden = true;
    closeHeaderFilterPopovers();
    buildFilterMenus();
    renderWeightControls();
  ...
  function stepBenchmarkHistory(direction) {
    ...
    if (!source.length || !currentHistoryEntry || historyRestoring) return false;
    const nextEntry = source.pop();
    const previousEntry = currentHistoryEntry;
    const resumeUrlSync = urlSyncReady;
    historyRestoring = true;
    urlSyncReady = false;
    try {
      restoreBenchmarkHistorySnapshot(nextEntry.snapshot);
      destination.push(previousEntry);
      ...
      console.error(`Unable to ${direction} the benchmark change`, error);
      return false;
    } finally {
  ...
  function historyControlKey(target) {
    if (!(target instanceof Element)) return "document";
    const rowId = target.closest("tr[data-id]")?.dataset.id || "";
    return [
      target.id || target.getAttribute("name") || target.getAttribute("aria-label")
        || target.dataset.sort || target.className || target.tagName,
      rowId,
    ].map(String).join(":");
  }

  function hasNativeUndo(target) {
  ...
  function applyAnalysisPayloadTransaction(nextPayload, nextPosition) {
    if (!SUPPORTED_URL_STATE_VERSIONS.includes(nextPayload?.v) || !POSITION_BY_KEY.has(nextPosition)) {
  ...
  function applySavedScenario(id) {
    ...
    try {
      ...
      if (!SUPPORTED_URL_STATE_VERSIONS.includes(nextPayload?.v)) throw new Error("Unsupported saved-state version.");
    } catch (error) {
  ...
  function robustnessWeightLabel(spec) {
    ...
    if (!keys.length) return spec.position === "ceo" ? "Equal (co-leaders share)" : "Equal organizations";
    if (keys.includes("comparability")) {
      const automatic = "Automatic (target eff. n " + spec.autoTargetEss + ")";
  ...
  function evaluateRobustnessSpec(spec, metadata = {}) {
    const base = {
      ...
      spec: {
        ...
        customWeightEditCount: spec.weightMode === "current"
          ? [...(spec.customWeights || new Map()).values()].filter((value) => Math.abs(Number(value) - 1) > 1e-9).length
          : 0,
        skipFilters: Boolean(spec.skipFilters),
  ...
  function robustnessCompletionLabel(report) {
    const results = [...report.core, ...report.source, ...report.measures, ...report.posting, ...report.dollars];
    const valid = results.filter((result) => result.status === "valid").length;
    return valid + " of " + results.length + " specifications produced usable results.";
  }
  ...
  function runModelRobustnessAnalysis() {
    const fingerprint = JSON.stringify([state.modelProfile, state.modelFocusWeights, state.modelTarget]);
    if (robustnessReport?.kind !== "model" || robustnessReport.fingerprint !== fingerprint) {
      const core = PREDICTIVE_MODEL.comparison.map((row) => withModelComparison(row, () => {
        const prediction = currentModelPrediction();
        const records = activeModelTrainingRecords();
        return { kind: "model", id: row.key, row, label: row.label,
          status: prediction ? "valid" : "invalid", warning: prediction ? "" : "Required profile inputs are invalid",
          rowCount: records.length, organizationCount: new Set(records.map((record) => record.organization)).size,
          q25: prediction?.distribution.quantile(.25), q50: prediction?.median, q75: prediction?.distribution.quantile(.75) };
      }));
  ...
  function runRobustnessAnalysis() {
    if (state.view === "model") { runModelRobustnessAnalysis(); return; }
    ...
    robustnessReport = {
      fingerprint, positionKey: baseSpec.position, position: positionDefinition().label,
      basis: baseSpec.inflationAdjusted ? "July 2026 USD" : "Original USD",
      dimensions: [...dimensions], core, source, measures, posting, dollars, current, generatedAt: new Date().toISOString(),
    };
    renderRobustnessReport();
    refs.robustnessStatus.textContent = robustnessCompletionLabel(robustnessReport);
  }
  ...
  function robustnessFilterLabel(spec) {
    if (spec.skipFilters) return "Not reapplied to the matched records";
  ...
  function robustnessPayloadSettingSignature(payload, key) {
    const keys = {
      sample: ["p", "i"],
      distribution: ["g"],
      weighting: ["w", "x", "d", "c"],
    ...
    return JSON.stringify(keys.map((token) => (
      payload && Object.hasOwn(payload, token) ? payload[token] : null
    )));
  }
  ...
  function showRobustnessTooltip(event, anchor, result, key, { applies = true } = {}) {
    ...
    if (result.kind === "model") {
      for (const [label, value] of [[robustnessQuantileLabel(key), money(result[key])],
        ...(state.modelRobustnessIntervals && result.intervals ? [[`${state.modelCompatibilityLevel}% ${expectationView() ? "estimation" : "percentile"} interval`, result.intervals[key].map(money).join("–")]] : []),
        ...
        const detail = document.createElement("div"); detail.append(resultElement("dt", "", label), resultElement("dd", "", value)); list.append(detail);
      }
      tooltip.replaceChildren(heading, list, resultElement("p", "", applies ? "Select to use this model." : "Current model."));
      tooltip.dataset.specId = result.id; tooltip.hidden = false; positionRobustnessTooltip(event, anchor); return;
    }
  ...
  function robustnessPointAriaLabel(result, key, applies = true) {
    if (result.kind === "model") return `${result.label}. ${robustnessQuantileLabel(key)} ${money(result[key])}. ${state.modelRobustnessIntervals && result.intervals ? `${state.modelCompatibilityLevel}% ${expectationView() ? "estimation" : "percentile"} interval ${result.intervals[key].map(money).join(" to ")}. ` : ""}${applies ? "Select to apply." : "Current model."}`;
  ...
  function applyRobustnessResult(result, key, { restoreFocus = false } = {}) {
    if (result.kind === "model") {
      ...
      hideRobustnessTooltip();
      selectModelComparisonMethod(result.id, { focus: false });
      refs.robustnessStatus.textContent = `Applied ${result.label}.`;
      if (restoreFocus) requestAnimationFrame(() => refs.robustnessResults.querySelector(
        `.robustness-spec-point[data-spec-id="${result.id}"][data-quantile="${key}"]`)?.focus({ preventScroll: true }));
      return;
    }
  ...
  function robustnessChart(report, descriptionId) {
    ...
    const locationOnly = report.kind === "model" && expectationView();
    const height = showIntervals ? rowHeight * (locationOnly ? 1 : 3) + 40 : locationOnly ? 76 : 132;
    const margin = { left: width < 420 ? 54 : 64, right: width < 420 ? 8 : 14, top: 8, bottom: 24 };
    const keys = locationOnly ? [["q50", "Median"]] : [
  ...
  function robustnessAuditTable(report, initialState = {}) {
    const initiallyOpen = Boolean(initialState.open);
    ...
    const collapsedLabel = "View all " + all.length + " specifications";
    const expandedLabel = "Hide all " + all.length + " specifications";
    const summary = resultElement("summary", "", collapsedLabel);
    const wrapperId = "robustness-all-specifications";
    summary.setAttribute("aria-controls", wrapperId);
    summary.setAttribute("aria-expanded", "false");
    details.append(summary);
    const wrapper = resultElement("div", "robustness-table-wrap");
    wrapper.id = wrapperId;
    wrapper.tabIndex = 0;
    wrapper.setAttribute("aria-label", "All robustness specifications; scroll to see every column and row");
    wrapper.scrollTop = Number(initialState.scrollTop || 0);
    wrapper.scrollLeft = Number(initialState.scrollLeft || 0);
    ...
    all.forEach((result) => {
      ...
      values.forEach((value, index) => {
        const isLabel = index === (modelReport ? 0 : 1);
        const cell = document.createElement(isLabel ? "th" : "td");
        if (isLabel) cell.scope = "row";
        cell.textContent = value;
    ...
    syncDisclosureState();
    let suppressInitialToggle = initiallyOpen;
    details.addEventListener("toggle", () => {
      syncDisclosureState();
      if (suppressInitialToggle) {
        suppressInitialToggle = false;
        return;
      }
      if (details.open) {
        requestAnimationFrame(() => wrapper.scrollIntoView({ block: "nearest", inline: "nearest" }));
      }
    });
    return details;
  }
  ...
  function renderRobustnessReport() {
    hideRobustnessTooltip();
    const previousAudit = refs.robustnessResults.querySelector(".robustness-audit");
    const previousAuditWrapper = previousAudit?.querySelector(".robustness-table-wrap");
    const auditState = {
      open: Boolean(previousAudit?.open),
      scrollTop: previousAuditWrapper?.scrollTop || 0,
      scrollLeft: previousAuditWrapper?.scrollLeft || 0,
      wrapperFocused: document.activeElement === previousAuditWrapper,
      summaryFocused: document.activeElement === previousAudit?.querySelector("summary"),
      panelScrollTop: refs.resultsPanel.scrollTop,
    };
    const measuredWidth = Math.round(refs.robustnessResults.getBoundingClientRect().width || 0);
    if (measuredWidth) robustnessResultWidth = measuredWidth;
    ...
    [
      ...
      [report.kind === "model" ? "Usable models" : "Usable core specs", validCore.length + " / " + report.core.length],
    ].forEach(([label, value]) => {
    ...
    const driver = report.kind === "model" ? null : robustnessLargestDriver(report);
    if (driver) {
    ...
    const audit = robustnessAuditTable(report, auditState);
    refs.robustnessResults.append(audit);
    const nextSummary = audit.querySelector("summary");
    if (auditState.open) {
      const nextWrapper = audit.querySelector(".robustness-table-wrap");
      const restoreAuditReadingPosition = () => {
        nextWrapper.scrollTop = auditState.scrollTop;
        nextWrapper.scrollLeft = auditState.scrollLeft;
        refs.resultsPanel.scrollTop = auditState.panelScrollTop;
        if (auditState.wrapperFocused) nextWrapper.focus({ preventScroll: true });
        else if (auditState.summaryFocused) nextSummary.focus({ preventScroll: true });
      };
      restoreAuditReadingPosition();
      requestAnimationFrame(restoreAuditReadingPosition);
    } else if (auditState.summaryFocused) nextSummary.focus({ preventScroll: true });
  }
  ...
  function updateRobustnessAvailability() {
    ...
    robustnessTab.hidden = false;
    modelDetailsTab.hidden = !modelView;
    if (!modelView && activeResultsTab === "model-details") {
    ...
    postingInput.disabled = !ceo || modelView;
    ...
    if (modelView) {
      if (activeResultsTab === "robustness") runModelRobustnessAnalysis();
      return;
    }
    if (robustnessReport && (robustnessReport.kind === "model" || robustnessReport.positionKey !== state.position)) {
      robustnessReport = null;
      robustnessResultWidth = 0;
      hideRobustnessTooltip();
      refs.robustnessResults.replaceChildren();
      refs.robustnessStatus.textContent = "Run the check to compare the selected alternatives for the current position.";
    }
    if (robustnessReport) {
      const stale = robustnessReport.fingerprint !== analyticalFingerprint();
      refs.robustnessStatus.textContent = stale
        ? "Settings changed since this check was run. Run it again to refresh the results."
        : robustnessCompletionLabel(robustnessReport);
      robustnessReport.current = evaluateRobustnessSpec(currentRobustnessSpec(), {
        id: "current", family: "Current", label: "Current salary specification",
      });
      renderRobustnessReport();
    }
  ...
  function populateModelSelect(select, levels, { includeAverage = true } = {}) {
    select.replaceChildren();
    if (includeAverage) {
      const average = document.createElement("option");
      average.value = "__average__";
      average.textContent = "Average category effect";
      select.append(average);
    }
  ...
  function populateModelControls({ normalize = usesModelCategories() } = {}) {
    ...
    definitions.forEach(([select, key, includeAverage]) => {
      ...
      // Switching models must preserve a valid profile, including a category
      // whose effect is prior-only in the selected training cohort.
      if (allLevels.includes(state.modelProfile[key]) && !supportedLevels.includes(state.modelProfile[key])) {
        supportedLevels.push(state.modelProfile[key]);
      }
      populateModelSelect(select, supportedLevels, { includeAverage });
      if (key === "focus_area") {
        const option = document.createElement("option"); option.value = "__mixture__"; option.textContent = "Custom focus shares"; select.append(option);
      }
    });
  }
  ...
  function syncModelControls() {
    ...
    const focusContainer = $("#model-focus-weights");
    if (!focusContainer.children.length) modelCategoryLevels("focus_area").forEach((label, j) => {
      const field = document.createElement("label");
      const title = document.createElement("span"); title.textContent = label;
      const input = document.createElement("input"); input.type = "number"; input.min = "0"; input.max = "100"; input.step = "any"; input.dataset.focusIndex = j;
      input.addEventListener("input", () => {
        state.modelFocusWeights[j] = Number(input.value);
        const total = state.modelFocusWeights.reduce((sum, value) => sum + value, 0);
        $("#model-focus-error").textContent = Math.abs(total - 100) > .01 ? `Shares total ${total.toFixed(1)}%; enter 100%.` : "";
        renderProfileResults(); scheduleUrlState();
      });
      field.append(title, input); focusContainer.append(field);
    });
    focusContainer.querySelectorAll("input").forEach((input, j) => { input.value = state.modelFocusWeights?.[j] || 0; });
    const bayesian = isBayesianMethod();
    const allowsAds = bayesian && state.modelMethod !== "bayesianExact";
    ...
    refs.modelSettingsNote.textContent = intercept ? "Profile inputs are unused." : "";
  }
  ...
  function syncControlsFromState() {
    ...
    refs.showJobAdIntervals.checked = state.showJobAdIntervals;
  ...
  function expandCompactUrlState(payload, semanticPosition = "") {
    if (!SUPPORTED_URL_STATE_VERSIONS.includes(payload?.v)) return payload;
  ...
  function restoreUrlState(payload, semanticPosition = "") {
    ...
    const compactVersion = SUPPORTED_URL_STATE_VERSIONS.includes(payload?.v);
    ...
    state.modelTarget = analysis.pm?.target === "e" ? "expectation" : "predictive";
    ...
    Object.entries(MODEL_PROFILE_URL_FIELDS).forEach(([key, code]) => {
      ...
      if (MODEL_CATEGORY_KEYS.includes(key)) {
        ...
        state.modelProfile[key] = finiteNumber(value, MODEL_PROFILE_DEFAULTS[key], minimum, maximum);
      }
    });
  ...
  function reset() {
    ...
    syncAxisControls();
    syncModelControls();
    refs.chartColor.value = state.chartColor; refs.showContours.checked = true; refs.showJobAdIntervals.checked = true;
  ...
  refs.chartViewTabs.forEach((tab) => {
    tab.addEventListener("click", () => activateChartView(tab.dataset.chartView));
    tab.addEventListener("keydown", (event) => {
      const view = chartViewKeyFromKeyboard(event);
      if (!view) return;
      event.preventDefault();
      activateChartView(view, { focus: true });
    });
  });
  ...
  [
    ...
    input.addEventListener("input", () => {
      ...
      const valid = input.value !== "" && Number.isFinite(value) && value >= minimum && value <= maximum;
      input.setAttribute("aria-invalid", String(!valid));
      if (!valid) {
        state.modelProfile[key] = null;
        renderProfileResults();
        return;
      }
      state.modelProfile[key] = value;
      renderProfileResults();
      scheduleUrlState();
    });
    input.addEventListener("change", () => {
      const value = Number(input.value);
      state.modelProfile[key] = Number.isFinite(value) && value >= minimum && value <= maximum
        ? value : MODEL_PROFILE_DEFAULTS[key];
      input.value = state.modelProfile[key];
  ...
  [
    ...
    if (key === "focus_area" && select.value === "__mixture__" && !state.modelFocusWeights) {
      state.modelFocusWeights = modelCategoryLevels("focus_area").map(() => 0);
    }
    syncModelControls();
    renderAll();
  }));
  $("#model-target").addEventListener("change", (event) => {
    state.modelTarget = event.target.value; state.hoverQuantile = null; renderAll();
  });
  $("#model-compatibility-level").addEventListener("change", (event) => {
    const value = Number(event.target.value);
    state.modelCompatibilityLevel = Number.isFinite(value) && value >= 50 && value <= 99 ? value : 89;
    event.target.value = state.modelCompatibilityLevel;
    renderAll();
  });
  $("#model-effect-units").addEventListener("change", (event) => {
    state.modelEffectUnits = event.target.value === "log" ? "log" : "percent";
    renderAll();
  });
  refs.modelResetProfile.addEventListener("click", () => {
    state.modelProfile = { ...MODEL_PROFILE_DEFAULTS };
    state.modelFocusWeights = null;
    syncModelControls();
    renderAll();
  });
  ...
  document.addEventListener("input", (event) => {
    if (event.target.matches?.('#model-settings input[type="number"][aria-invalid="true"]')) return;
    scheduleUrlState();
  });
  ...
  document.addEventListener("input", (event) => {
    if (event.target.matches?.('#model-settings input[type="number"][aria-invalid="true"]')) return;
    recordBenchmarkHistory(historyControlKey(event.target));
  });
  document.addEventListener("change", (event) => {
    recordBenchmarkHistory();
    if (historyCoalesceKey === historyControlKey(event.target)) historyCoalesceKey = "";
  });
  ...
  document.addEventListener("keydown", (event) => {
    const nativeUndo = hasNativeUndo(event.target);
    if (!nativeUndo && !event.altKey && (event.ctrlKey || event.metaKey)) {
      const key = event.key.toLowerCase();
      const direction = key === "z" ? (event.shiftKey ? "redo" : "undo")
        : key === "y" && !event.shiftKey ? "redo" : "";
      if (direction) {
        if (stepBenchmarkHistory(direction)) event.preventDefault();
        return;
      }
    }
